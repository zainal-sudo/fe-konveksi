...
const searchPo = async (q: string) => {
  ...
  try { poOptions.value = await bpbFormApi.getPoOptions(q || ""); }
  catch { /* silent */ } finally { poLoading.value = false; }
};
...
const selectPo = async (po: any) => {
  ...
  try {
    ...
    if (res && res.length > 0) {
      ...
      toast.success(`Berhasil memuat ${form.value.detail.length} item dari PO ${poNo}`);
    } else {
      toast.warning("PO ditemukan, tetapi tidak ada item yang tersisa untuk diterima.");
      form.value.detail = [];
    }
  } catch (error: any) {
...
const searchGudang = async (q: string) => {
  ...
  try { gudangOptions.value = await bpbFormApi.getGudang(q || ""); }
...
const validateSave = () => {
  if (!form.value.poNomor) {
    toast.warning("Pilih Nomor PO terlebih dahulu."); return;
  }
  if (!form.value.gdgKode) {
    toast.warning("Pilih gudang tujuan."); return;
  }
  if (form.value.detail.length === 0) {
    toast.warning("Tidak ada item dari PO."); return;
  }
  if (activeRows.value === 0) {
    toast.warning("Minimal satu item harus memiliki qty > 0."); return;
  }
  for (const d of form.value.detail) {
    const sisa = d.qtyPo - d.qtySudahTerima;
    if ((d.qty || 0) > sisa) {
      toast.warning(`Qty terima "${d.brgNama}" (${d.qty}) melebihi sisa PO (${sisa}).`);
      return;
    }
  }
...
<template>
  <BaseForm
    :title="isEdit ? 'Ubah Bukti Penerimaan Barang' : 'Penerimaan Barang Baru (BPB)'"
        ...
        <div class="grid-fields">
          ...
          <div class="f-row full-col">
            ...
            <div class="search-group">
              <input
                type="text" :value="form.poNomor"
                class="f-inp readonly-bg po-inp"
                readonly placeholder="Klik untuk memilih PO..."
              />
              <button class="btn-srch btn-blue" type="button"
                @click="openPoModal" :disabled="isEdit || poLocked">
                <IconSearch :size="14" />
              </button>
              <button v-if="poLocked && !isEdit"
                class="btn-reset" type="button" @click="resetPo">
                ✕ Ganti
              </button>
            </div>
          </div>
          ...
          <div class="f-row full-col">
            ...
            <div class="search-group">
              ...
              <input type="text" :value="form.gdgNama"
                class="f-inp readonly-bg" readonly placeholder="Pilih gudang..." />
              <button class="btn-srch btn-green" type="button" @click="openGudangModal">
                <IconSearch :size="14" />
              </button>
            </div>
          </div>
      ...
      <div class="tbl-wrap">
        <table class="dtl-tbl">
          ...
          <tbody>
            ...
            <tr
              v-for="(row, idx) in form.detail" :key="idx"
            ...
            >
              ...
              <td class="tr text-orange-darken-3">{{ fmtQty(row.qtySudahTerima) }}</td>
              ...
              <td>
                <input
                  ...
                  :class="{ 'qty-over': row.qty > (row.qtyPo - row.qtySudahTerima) }"
          ...
          <tfoot v-if="form.detail.length > 0">
            <tr class="tfoot-row">
              ...
              <td class="tr font-weight-bold text-green-darken-4 pr-2">{{ fmtQty(totalQty) }}</td>
              <td></td>
            </tr>
          </tfoot>
        </table>
    ...
    <v-dialog v-model="showSaveDialog" max-width="340">
      <v-card rounded="lg">
        ...
        <v-card-text class="pa-4 text-body-2">
          Simpan penerimaan barang dari PO <strong>{{ form.poNomor }}</strong>
          ke gudang <strong>{{ form.gdgNama }}</strong>?<br/>
          <span class="text-caption text-grey">Total qty: {{ fmtQty(totalQty) }} | {{ activeRows }} item</span>
        </v-card-text>
...
.po-inp { font-weight: 700; color: #2e2e7d !important; }
.btn-srch {
...
.btn-reset {
  height: 28px; padding: 0 8px; background: #ef5350; color: white;
...
.row-zero td { color: #bdbdbd !important; }
...
.qty-over { background: #ffebee !important; border-color: #ef9a9a !important; color: #c62828 !important; }
